...
class UploadService {
    ...
    public $sizeLimit = 5242880;         // Max file upload size in bytes (5MB)
    ...
    private function errorCodeToMsg($code) {
        switch($code) {
            case UPLOAD_ERR_INI_SIZE:
                $message = 'Bestand is te groot (max. 5MB).';
                break;
            case UPLOAD_ERR_PARTIAL:
                $message = 'Het bestand is slechts gedeeltelijk geüpload.';
                break;
            case UPLOAD_ERR_NO_FILE:
                $message = 'Geen bestand geüpload.';
                break;
            case UPLOAD_ERR_NO_TMP_DIR:
                $message = 'Tijdelijke map ontbreekt.';
                break;
            case UPLOAD_ERR_CANT_WRITE:
                $message = 'Kon bestand niet schrijven.';
                break;
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null) {
        ...
        if (!empty($this->allowedExtensions)) {
            if (!$this->checkExtension($this->fileExtension, $this->allowedExtensions)) {
                $this->setErrorMsg('Ongeldig bestandstype.');
                return false;
            }
        }
